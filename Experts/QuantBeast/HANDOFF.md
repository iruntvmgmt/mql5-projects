...
- **Stale-binary rediscovery**: `qb-live-20260724-02`'s first verify attempt showed 106/1 (the old TEST 37 failure) despite the fresh 11:52 compile -- confirmed the terminal was still running a cached binary despite chart detach/reattach, exactly as first found earlier this session. A genuine terminal restart (not just chart reattach) resolved it; `cmd_verify` itself also had two real scoping bugs found and fixed in the process (matched the *first* self-test-summary/lease line in the whole day's cumulative log instead of the one from *this specific attach* -- see `TestEvidence/deployment_automation_20260724/DEPLOYMENT_CYCLE_EVIDENCE.md` addendum for full detail).
