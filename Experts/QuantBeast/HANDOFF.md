...
### 2026-07-22/23 — Production-readiness closure sprint: TP V1 freeze, TP V2, infrastructure audit, restricted demo candidate, unified evidence
**Commits:** `6ce0a41`, `ee7db48`, `026e91c`, `ed6bb18`, `acefa09`, `e04d8aa`, `8f33449`, `cce55a2`, `a17484c`. **Tag:** `quantbeast-tp-v1-research-freeze-20260722` @ `953c2d0`.

- Full report: `TestEvidence/production_readiness_tp_v2_20260722/final_readiness/FINAL_PRODUCTION_READINESS_REPORT.md`.
- **TP V1 frozen**, tagged at the exact evidence-producing commit; `LifecycleVersion` stamped on every diagnostic/outcome row going forward so V1/V2 evidence can never be pooled. V1 stays wired, unchanged, for future passive verification.
- **TP V2 built from scratch** (`Include/QuantBeast/Strategies/TrendPullbackV2Engine.mqh`): an 8-state lifecycle with its own decoupled trend-integrity/invalidation model, directly addressing the architectural mismatch behind V1's 11/16 rejection mode (a single instantaneous `regime.structure` reading was never designed to arbitrate a multi-bar sequential hypothesis). Full spec pre-registered before implementation (`TestEvidence/.../tp_v2_spec/`). Wired as a 5th strategy (`QB_STRAT_IDX_TPV2`) behind `InpEnableTPV2Experimental=false` (default) -- structurally zero reachable path to a valid signal while off. Tests 75-93 (18 new tests + 1 config-validation test); a lifecycle reset-ordering bug found and fixed during test development (Tests 79/81/84 initially failed).
- **Two genuine array-bounds defects** found and fixed while wiring TP V2 as the 5th strategy: `KillSwitchState.strategy_kill` hardcoded `bool[4]`, and the arbitration loop's `StrategySignal candidates[8]` -- both would have silently overrun.
- **Genuine pre-existing tracker defect** found (unrelated to TP V2 itself): `CTPOutcomeTracker::WriteRow()` skipped its own finalized-event bookkeeping when the journal file couldn't be opened, discovered via Wine file-handle flakiness unique to the ten-instances-in-one-process self-test pattern. Fixed.
- **Infrastructure audit** (`TestEvidence/.../infrastructure_audit/INFRASTRUCTURE_AUDIT.md`): surveyed the existing extensive verification record rather than re-deriving it; added `QBProductionConfigurationValid()` (rejects nonfinite/negative/zero/dangerously-permissive safety-critical inputs, fails `OnInit` closed) and `QBLogResolvedProductionConfiguration()` (startup config log) -- both genuine, previously-missing gaps; closed the "no per-strategy/direction/session/regime report" gap with `Tools/strategy_performance_report.py`.
- **Restricted all-strategy demo candidate prepared, not activated**: `XAUUSD_Conservative_Demo_AllStrategy.set` (BO+FBO+MR+TPV2, TPV2 experimental still off). `QBLiveStrategySetAllowed()` (FBO-only live gate) deliberately left untouched -- loading this preset today fails closed exactly like any other non-FBO-only config, which is correct, not a bug.
- **Unified all-strategy evidence** (`TestEvidence/.../unified_strategy_matrix/`): 6 XAUUSD M5 Model=4 windows (4 reused/regression-reproduction, 2 untouched), 4.93M ticks, 17,220 signal-decision rows, run last per explicit build-then-test sequencing. FBO and MR organically `ACCEPTED` in 6/6 windows; BO in 1/6; TP V1 in 0/6 (unchanged); TP V2 never accepts (experimental off) but its full lifecycle including `TRIGGERED` is organically reachable in 3/6 windows (9 episodes, 4 of which passed every geometry/spread/confidence gate). 48 completed Shadow trades, 100% traced candidate-through-exit.
- Validation: compile `0 errors, 0 warnings`; self-tests `96 passed, 0 failed`.
- Safety: zero open positions/orders verified before every terminal-sensitive action; `InpEnableTPV2Experimental=false` throughout; no live or Challenge-mode run; **no broker orders transmitted**. Readiness remains exactly `READY FOR SHADOW MODE`.
- Exact remaining blockers (fixable-now vs. requires-forward-time, split): see report section 14. Strongest next-step candidates: (1) a Shadow-only TP V2 experimental-on run for arbitration/risk reachability evidence, (2) an evidence-justified operator decision on whether to add MR to the live gate.

### 2026-07-23 (session 2) — Kill-switch restore-warning fix (D011 follow-up)

- Scope: single defect-group fix, picked from the open items surfaced by the follow-on production-readiness sprint (`TestEvidence/production_readiness_tp_v2_20260722/final_readiness/FOLLOWON_SPRINT_FINAL_REPORT.md`) and the FBO+MR Conservative-Demo activation that followed it (`TestEvidence/production_readiness_tp_v2_20260722/run_manifests/run_live01_20260723_fbo_mr_activation.md`, `DECISION_LOG.md` D011). D011 found that `LoadKillSwitchState()` restoring a stale `emergency`/kill latch from a prior session's GlobalVariable was completely silent at `OnInit` -- the EA blocked all entries and `ProcessKillSwitchActions()` looped `CancelAllPending()`/`CloseAllPositions()` indefinitely with no log explaining why, discovered only through live investigation during the July 23 activation. Deliberately deferred at the time per explicit user instruction ("note that bug incase it returns we should fix it").
- Fix: new pure formatter `QBKillSwitchRestoreWarning(const KillSwitchState &state)` in `Include/QuantBeast/Core/StateStore.mqh` (right after `LoadKillSwitchState()`) builds a detailed warning string whenever any of `emergency`/`entry_kill`/`symbol_kill`/`cancel_all`/`flatten_all` is set on a just-restored state, quoting `emergency_reason` and naming the `QB_Emergency`/`QB_Kill*` GlobalVariables to check. `OnInit()` in `Experts/QuantBeast/QuantBeastEA.mq5` calls it immediately after `g_KillSwitch.RestoreState(savedKill)` and logs a non-empty result via `QBLogError()`. This is strictly additive observability -- no change to `IsEntryKill()`/`IsFlattenAll()`/`IsCancelAll()`/`ProcessKillSwitchActions()` control flow, per `AGENTS.md`'s "never weaken kill-switch controls" rule. The companion hardening idea from D011 (an explicit re-acknowledgment gate before a *restored* latch can execute broker actions) was deliberately NOT implemented -- see `DECISION_LOG.md` D012's Reason section: it risks delaying a genuinely-needed flatten during a real restart-mid-emergency scenario, an unacceptable safety regression for an incident that caused zero trading harm and is already fully addressed by making the state visible.
- Test: new self-test `QBTestKillSwitchRestoreWarning` (`Include/QuantBeast/Testing/SafetyTests.mqh`), registered as TEST 103. Proves a clean state stays silent, an emergency-latched state warns and includes the reason string, and an entry-kill-only state (no emergency) also warns -- covering both halves of the original silent-failure mode.
- Verification: compiled via the documented `wine start /Unix metaeditor64.exe /compile:"MQL5\Experts\QuantBeast\QuantBeastEA.mq5"` workaround (`AGENTS.md` "Compilation contract") -- `0 errors, 0 warnings` (`logs/metaeditor.log`, 2026.07.23 23:11:24, 43966 ms elapsed). Self-test-only tester run using the existing `Profiles/Tester/QuantBeast.SelfTestDetail.20260722.ini` (Diagnostic mode, `InpPersistState=false`, `InpUseGlobalVars=false`, no journals, unmodified) confirmed via `Tester/Agent-127.0.0.1-3000/logs/20260723.log`: `Self-tests complete: 106 passed, 0 failed` (up from the pre-existing 105), `TEST 103 PASS: Kill-switch restore warning cleanIsSilent=yes emergencyWarns=yes entryOnlyWarns=yes`, zero `FAIL` lines, `TEST 102` and all prior tests still green -- no regression. No historical-window evidence backtest was run for this fix (not applicable -- this is a code-correctness defect, not a strategy-behavior change); per the user's own standing preference, evidence-gathering tester windows are reserved for multi-part sprints and are run last, not for a single self-contained defect fix like this one.
- Files changed: `Experts/QuantBeast/QuantBeastEA.mq5`, `Include/QuantBeast/Core/StateStore.mqh`, `Include/QuantBeast/Testing/SafetyTests.mqh`, plus this `HANDOFF.md` entry and `TestEvidence/production_readiness_tp_v2_20260722/DECISION_LOG.md` (D012).
- Commit: `f0a153e`.
- Next task: get explicit go-ahead to commit this fix (message to match this entry's title), then consider the remaining open items from the follow-on sprint report: BO's BUY-side organic acceptance is still unobserved, TP V2 needs a second organic Shadow lifecycle plus a real restart-with-open-position proof, and (separately, lower priority) whether the D011/D012 re-acknowledgment gate should be revisited if a restored-latch incident recurs in practice.

### 2026-07-24 -- Deployment-automation workflow (lease gate + Tools/quantbeast_deploy.py + quantbeast_watchdog.py)

- Scope: user requested a deployment-automation system (compile -> test -> package -> deploy -> verify -> rollback) with a fail-closed deployment identity gate in the EA. Full plan researched and written before implementation (three parallel Explore passes covering OnInit/gate conventions, MT5 terminal topology, and AGENTS.md/DECISION_LOG constraints) -- see the user's original detailed spec for the full ask.
- Key constraint findings that reshaped the plan from the original spec: only one MT5 install exists on this Mac (no blue/green terminal slots without a brand-new install -- user chose to reuse the single instance with pre-flight isolation checks instead); no working precedent for `/config`/`[StartUp]` EA auto-attach (Phase 0 diagnostic below, negative result); no precedent for scripted `terminal64.exe` process kill/restart; automated chart-attach (`chart_apply_template`) was already hard-blocked once by Claude Code's own auto-mode classifier (D011) -- so chart-attach and flipping `InpAcknowledgeLiveBrokerRisk` stay a deliberate manual step, matching every prior live/demo activation in this project.
- Built: `QBDeploymentLeaseValid()` (`QuantBeastEA.mq5`, `DeploymentLease` struct in `Include/QuantBeast/Core/Types.mqh`, `QBReadDeploymentLease()` in `Include/QuantBeast/Core/Diagnostics.mqh`) -- fail-closed gate wired into `OnInit()`, scoped to `requestedLiveMode` only (Shadow/Diagnostic self-test and evidence tester runs unaffected). New self-test TEST 104 (in `QuantBeastEA.mq5`, inline like TEST 102 -- not `SafetyTests.mqh`, since `QBDeploymentLeaseValid` is defined after that file is `#include`d and MQL5 requires prior definition). `Tools/quantbeast_deploy.py` (prepare/preflight/deploy/verify/status/rollback) and `Tools/quantbeast_watchdog.py` (read-only, alert-only).
- Phase 0 (`TestEvidence/deployment_automation_20260724/PHASE0_CONFIG_DIAGNOSTIC.md`): bounded, disposable-target empirical test of `terminal64.exe /config:...` via both `wine start /Unix` and direct invocation. Negative result both times -- zero observable effect while a primary instance is running. Fell back exactly per the user's own spec's contingency (manual attach + MT5's own EA-restore-on-restart behavior).
- Compile 0 errors/0 warnings; self-tests 107 passed/0 failed (Diagnostic-mode tester run) before any live activity.
- Dry-run: full `prepare -> preflight -> deploy -> status -> verify -> rollback` cycle against a disposable fake lease (server=DRYRUN-NotARealServer, login=1), confirming the whole pipeline mechanically before touching the real account. Found and fixed three real bugs purely from running the tool for real: a `--once` watchdog logic error (never actually scanned anything), a flaky `subprocess.run(capture_output=True)` hang on the wine launcher, and an infinite-recompile-loop in `prepare`'s self-test-freshness check (fixed with incremental-build skip logic).
- Real deploy: `qb-live-20260724-01` against Coinexx-Demo (login 871221), roster BO+FBO+MR+TPV2 (TPV2 experimental off, TP V1 excluded), 0.01 lot/1 position caps. Operator performed the manual attach + `InpAcknowledgeLiveBrokerRisk=true` step. Real terminal log confirms the lease gate worked exactly as designed: `── Resolved Deployment Lease ── found=yes id=qb-live-20260724-01 ... valid=yes`. Found and fixed two more real bugs during this real run: `verify`/watchdog were reading the wrong log directory (terminal-root `Logs/` only carries Journal-level lifecycle events; the EA's own `QBLogInfo`/`QBLogError` output goes to `MQL5/Logs/`, confirmed empirically), and TEST 37 ("Live strategy allowlist") has a non-hermetic sub-assertion that hardcodes an expectation `InpBO_DemoAuthorized=false` -- it fails whenever BO is deliberately demo-authorized (which this deployment does on purpose), a pre-existing test-design coupling, not a regression. `verify` now treats exactly that single-failure signature as expected. Also found the watchdog couldn't distinguish self-test-phase log noise (which deliberately constructs synthetic emergency/failure scenarios, e.g. a real `EMERGENCY: Equity floor breached` line from TEST 29's own construction) from genuine runtime alerts -- fixed by excluding the self-test execution window from alert matching. Full writeup: `TestEvidence/deployment_automation_20260724/DEPLOYMENT_CYCLE_EVIDENCE.md`.
- **Decision requiring disclosure**: the canonical roster authorizes BO for live/demo trading, but BO is rated `SHADOW_READY` (not `DEMO_READY`) in the project's own evidence-based readiness table. This was surfaced to the user only after the real attach was already live-armed, not before -- a process gap for next time. User's explicit, informed decision: leave it as deployed (their demo account, bounded 0.01-lot risk). See DECISION_LOG.md D013.
- Compile: 0 errors, 0 warnings (unchanged from the compile at the top of this session -- no source changes were needed between the kill-switch fix and this entry once the lease-gate code itself was in). Self-tests: 107 passed, 0 failed in Diagnostic mode; 106 passed, 1 failed (the known TEST 37 artifact) on the real live attach with BO authorized -- both expected and understood, not regressions.
- Files changed: `Experts/QuantBeast/QuantBeastEA.mq5`, `Include/QuantBeast/Core/Types.mqh`, `Include/QuantBeast/Core/Diagnostics.mqh`, `Experts/QuantBeast/Tools/quantbeast_deploy.py` (new), `Experts/QuantBeast/Tools/quantbeast_watchdog.py` (new), plus this entry, `DECISION_LOG.md` D013, and the `TestEvidence/deployment_automation_20260724/` evidence pack.
- Next task: decide whether TEST 37 itself should be made hermetic (accept an explicit `boDemoAuthorized` parameter instead of reading the live input) so it stops depending on which roster is loaded -- currently worked around in `verify`, not fixed at the source. Also: the BO/DEMO_READY gap noted above should inform whether BO stays in future canonical-roster deployments or gets scoped back once organic evidence catches up to FBO/MR's bar.

### 2026-07-24 (session 3) -- Blanket demo-only authorization: full activation batch + two more silent-restore fixes

- Scope: user granted standing, global authorization to proceed with every open item across HANDOFF.md/KNOWN_LIMITATIONS.md/DECISION_LOG.md, explicitly citing Coinexx-Demo carries no real-money risk. This session closed out the TEST 37 hermeticity gap (fixed for real this time, not just worked around in `verify`), wired the dead `AllocationEngine::RecordOutcome()`, added a `InpAcknowledgePendingOrderRisk` gate, added a `QB_BROKER_TIER_CHALLENGE_DEMO` tier, and genuinely live-deployed and verified all three new capabilities plus TP V2 experimental on Coinexx-Demo for the first time in this project's history. Along the way, a user-prompted investigation (via manually reading the Experts log) surfaced two more real, previously-unknown silent-restore/operational bugs. See DECISION_LOG.md D014-D016 for full decision records.
- **TEST 37 fixed at the source**: `boUnauthorizedRejected` now calls `QBStrategyAllowlistCheck()` directly with an explicit `demoAuthorized=false`, instead of `QBLiveStrategySetAllowed()` (which read the live `InpBO_DemoAuthorized` input and assumed its shipped-false default). Matches the `tpv2AllowedWhenAuthorized` sub-check's existing hermetic pattern in the same test.
- **`AllocationEngine::RecordOutcome()` wired**: `CTradeJournal::LogTrade()` (`Include/QuantBeast/Analytics/TradeJournal.mqh`) now returns the `rMultiple` it already computes; both close paths (`ProcessShadowCloseEvents`, the real `OnTradeTransaction` close handler) feed it to `g_Allocator.RecordOutcome()`. TEST 105 proves `ALLOC_PERFORMANCE` now genuinely differentiates weights (`boWeight=1.500` vs `fboWeight=0.500` after a +2R BO outcome vs 0R FBO baseline) using fresh, unconfigured instances -- no file I/O, no production-state pollution.
- **New `InpAcknowledgePendingOrderRisk` gate** (`Include/QuantBeast/Core/Configuration.mqh`): pending orders were unconditionally blocked at `OnInit` with zero unlock path. `QBLiveExecutionSetAllowed()` now permits `useMarketOrders=false`/stop/limit/`maxPendingOrders>0` configurations only when explicitly acknowledged, following the exact `InpAcknowledgeLiveBrokerRisk` pattern. TEST 38 rewritten to cover with-ack-accepted and without-ack-rejected for every pending configuration.
- **New `QB_BROKER_TIER_CHALLENGE_DEMO` tier**: `QBCurrentBrokerTier()` now classifies `QB_MODE_CHALLENGE_LIVE` by connected account exactly like Conservative Live already does -- a real account still always maps to the never-sanctioned `CHALLENGE_LIVE` tier (AGENTS.md's real-account prohibition untouched), a verified demo account now maps to `CHALLENGE_DEMO`, sanctioned by `QBStrategyAllowlistCheck` alongside `CONSERVATIVE_DEMO`. TEST 102 fixed (its `challengeIsChallengeTier` assertion hardcoded the old unconditional behavior -- now computes the expected tier from the actually-connected account, same hermeticity lesson as TEST 37). New TEST 106 proves the Challenge-Demo allowlist path.
- **`Tools/quantbeast_deploy.py` roster presets**: `CANONICAL_ROSTER` (now `InpEnableTPV2Experimental=true`, matching the user's original spec rather than the earlier conservative deviation), `PENDING_ORDER_ROSTER`, `CHALLENGE_ROSTER`, selectable via `prepare --roster {canonical,pending-orders,challenge}`.
- Compile: 0 errors, 0 warnings. Self-tests: 109 passed, 0 failed (up from 107) after this batch.
- **Real deploy cycles, all verified via real terminal log inspection** (not just self-test evidence): `qb-live-20260724-02` (canonical, TPV2 experimental=true -- first time TP V2 ever live-armed), `qb-live-20260724-03-pending` (`UseMarketOrders=no UseStopOrders=yes UseLimitOrders=yes` confirmed in the real resolved-config log -- first time pending orders ever live-armed), `qb-live-20260724-04-challenge` (`Mode: QB_MODE_CHALLENGE_LIVE` / `BrokerTier=QB_BROKER_TIER_CHALLENGE_DEMO` confirmed -- first time Challenge Mode ever live-armed). Zero broker orders transmitted across all three; every window ended flat.
- **Stale-binary rediscovery (attach-method corrected 2026-07-24)**: `qb-live-20260724-02`'s first verify attempt showed 106/1 (the old TEST 37 failure) despite the fresh 11:52 compile -- confirmed the terminal was still running a cached binary, matching the same stale-binary pattern observed earlier this session. This was originally attributed to "chart detach/reattach"; the operator subsequently corrected the record -- the actual step performed was an MT5 **profile load** (attach method `PROFILE_LOAD`), not a genuine Remove EA -> fresh manual reattachment (`EA_REMOVE_REATTACH`, which was never tested here). Empirically supported facts only: the `.ex5` on disk had changed; loading the profile did not replace the running old binary; a full terminal restart subsequently loaded the fresh binary. Terminal restart is the currently-proven reliable refresh procedure, but it is not proven to be the only valid one -- genuine `EA_REMOVE_REATTACH` remains untested. `cmd_verify` itself also had two real scoping bugs found and fixed in the process (matched the *first* self-test-summary/lease line in the whole day's cumulative log instead of the one from *this specific attach* -- see `TestEvidence/deployment_automation_20260724/DEPLOYMENT_CYCLE_EVIDENCE.md` for the full corrected writeup).
- **Silent risk-lock restore found and fixed** (DECISION_LOG.md D015): user-prompted investigation of manually-noticed "Signal rejected by risk engine: Drawdown lock active" lines found 7 real rejections on the live account spanning 2026-07-23 23:20 through 2026-07-24 11:25 -- zero trades in that entire window. Root cause: `m_drawdownLockActive` restores unconditionally from persisted GlobalVariable state with no date/period gating (unlike daily/weekly locks, which auto-clear on rollover) and `CRiskEngine::ResetState()` -- the only clearing code -- is never called in production. Fixed with `QBRiskLockRestoreWarning()` (`Include/QuantBeast/Risk/RiskEngine.mqh`), logged loudly at `OnInit` right after `InitDailyTracking()`. TEST 107 proves it. Self-tests: 110 passed, 0 failed.
- **GlobalVariable persist-on-detach discovered** (DECISION_LOG.md D016): clearing `QB_KillEntries_871221_XAUUSD` via the GUI while the EA was still attached did not stick -- reappeared as 1.0 after detach/reattach. Root cause: the EA reads kill-switch/risk GlobalVariables once at `OnInit` and holds them in memory; `OnDeinit()` unconditionally re-persists the in-memory (still-stale) state on every detach, silently clobbering any manual edit made while attached. Not fixed in code (judged correct-by-design for a stateful process) -- documented as the required operational sequence instead: **detach first, then edit the GlobalVariable, then reattach**. This retroactively explains why D011's original fix worked (operator happened to remove the EA before editing that time).
- After both fixes and the correct clear sequence, `qb-live-20260724-05-longrun` (canonical roster, 14-day lease) verified fully clean: no restore warnings of any kind, 110/0 self-tests, real risk state confirmed unlocked (`dailyStart=999.64 weeklyStart=997.71 HWM=1022.40`, no lock warning fired), 0 positions/0 orders. This is the first deployment all day genuinely able to trade.
- **Exact current configuration of `qb-live-20260724-05-longrun`** (from its own manifest/`.set`/lease, not inferred): effective mode `InpMode=2` (`QB_MODE_CONSERVATIVE_LIVE`; account is Coinexx-Demo, so broker tier resolves to `CONSERVATIVE_DEMO`, not a real-account tier). Market-vs-pending routing: `InpUseMarketOrders=true`, `InpUseStopOrders=false`, `InpUseLimitOrders=false`, `InpMaxPendingOrders=0` -- **market-orders-only; the pending-order capability is not active in this deployment**. Challenge Mode: **not active** -- `InpAcknowledgeChallengeRisk=false` and mode is 2, not `QB_MODE_CHALLENGE_LIVE`. Roster: BO/FBO/MR/TPV2 all `Enabled=true` and `DemoAuthorized=true`; TP V1 (`InpTP_Enabled`) permanently `false`. TP V2 experimental: `InpEnableTPV2Experimental=true` (on). Risk caps: `InpRiskPercent=0.10%`, `InpMaxRiskPerTrade=0.25%`, `InpMaxPositions=1`, `InpMaxTotalExposureLots=0.01`, `InpMaxSpreadPoints=20`, `InpDailyLossLimitPct=1.0%`, `InpWeeklyLossLimitPct=2.0%`, `InpMaxDrawdownPct=3.0%`, `InpMaxConsecLosses=1`, `InpMaxConsecutiveBrokerFailures=1`. Lease expiry: `2026-08-07 23:16:39 UTC` (build `1.00-20260701`, git commit `a71af41`). **The pending-order (`qb-live-20260724-03-pending`) and Challenge-Demo (`qb-live-20260724-04-challenge`) capabilities were each independently deployed, verified, and torn down as separate, short-lived verification windows earlier the same day -- they are not simultaneously active in the standing long-run deployment.** Only the canonical roster (BO+FBO+MR+TPV2-experimental, market-orders-only, Conservative Live/Demo) is currently live.
- Files changed: `Experts/QuantBeast/QuantBeastEA.mq5`, `Include/QuantBeast/Core/Configuration.mqh`, `Include/QuantBeast/Analytics/TradeJournal.mqh`, `Include/QuantBeast/Risk/RiskEngine.mqh`, `Include/QuantBeast/Testing/SafetyTests.mqh`, `Experts/QuantBeast/Tools/quantbeast_deploy.py`, plus this entry, `DECISION_LOG.md` D014-D016, and `KNOWN_LIMITATIONS.md`.
- Next task: `qb-live-20260724-05-longrun` is live for the next 14 days (expires 2026-08-07) -- BO BUY-side and a second TP V2 organic lifecycle (previously separate bounded-backtest tasks) are expected to fall out of this ongoing observation naturally; check back periodically with `Tools/quantbeast_deploy.py status` / `Tools/quantbeast_watchdog.py --once`. **Engineering follow-up (elevated 2026-07-24): build an audited, operator-facing risk-lock reset command** (mirroring `CMD_CLEAR_KILL_STATE`, wiring `CRiskEngine::ResetState()` behind it, with the action logged the same way kill-switch clears are) so a stale drawdown lock no longer requires manual GlobalVariable surgery. The current detach -> edit GlobalVariables -> reattach sequence is an acceptable *emergency* operational procedure (proven to work, D016) but is not production-grade administration -- no audit trail, no confirmation step, easy to get the ordering wrong. Not done this session (scope control); see `DECISION_LOG.md` D015/D017.

### 2026-07-24 (correction) -- attach-method retraction for the stale-binary finding

- Scope: pure documentation correction, no code change. The user identified that this session's earlier account of the `qb-live-20260724-02` stale-binary incident (recorded above and in `TestEvidence/deployment_automation_20260724/DEPLOYMENT_CYCLE_EVIDENCE.md`) mischaracterized what they had actually done: they had loaded an MT5 **profile**, not performed a genuine chart-level Remove EA -> fresh manual reattachment.
- Correction applied: reworded the "Stale-binary rediscovery" bullet above, the `DEPLOYMENT_CYCLE_EVIDENCE.md` "Second real deploy" section, and `DECISION_LOG.md` (new entry D017) to state only the empirically supported facts -- the `.ex5` on disk had changed, loading the profile did not replace the running old binary, a full terminal restart subsequently loaded the fresh binary, and genuine `EA_REMOVE_REATTACH` behavior was never tested. Terminal restart remains the only *proven* reliable refresh procedure; it is not proven to be the only *valid* one.
- Also clarified `qb-live-20260724-05-longrun`'s exact resolved configuration (see the bullet above) so the pending-order and Challenge-Demo capabilities' one-time verification deployments are not mistaken for standing state.
- Files affected: this entry, `DECISION_LOG.md` (D017, new), `TestEvidence/deployment_automation_20260724/DEPLOYMENT_CYCLE_EVIDENCE.md`, `KNOWN_LIMITATIONS.md`, `Experts/QuantBeast/Tools/quantbeast_deploy.py` (manual-step instructions annotated with the corrected refresh-procedure guidance).
- Next task: none new -- `qb-live-20260724-05-longrun` remains live per the entry above; the audited risk-lock-reset command remains the standing engineering follow-up.

### 2026-07-24 (later) -- Level-1 Edge Certification Sprint (Stage 1), and a live-blocking RiskEngine bug found + fixed along the way

- Scope: user asked how much engineering work remains to remove caps/locks for aggressive growth. Pushed back on the framing (raising caps is trivial; removing risk locks removes the only instrument that would show a failing strategy before it compounds losses; no strategy here has enough sample size yet to justify either). User then shared an external ("codex") proposal for a rigorous, anti-overfitting historical edge-certification sprint (walk-forward, Monte Carlo, Deflated Sharpe Ratio, Probability of Backtest Overfitting). Agreed to stage it rather than build the whole apparatus at once.
- **Stage 1 complete**: `TestEvidence/level1_edge_certification_20260724/RESEARCH_TRIAL_LEDGER.md` -- full inventory of every historical window and parameter/strategy variant this project has ever touched (31 distinct real-tick windows 2024.10.07-2026.07.20, conservative floor of >=120 distinct trials, since one window alone -- 2026.06.22-23 -- was reused across ~20 named test suites). `TestEvidence/level1_edge_certification_20260724/CERTIFICATION_WINDOW_SELECTION.md` -- originally selected 7 regime-diverse windows from 2019-2023 plus a 2024 holdout, all pre-dating this project's history.
- **Stage 2 complete**: `Tools/quantbeast_certification.py` -- expectancy, profit factor, win rate, avg win/loss, per-trade Sharpe/Sortino, max drawdown (currency), recovery factor, longest losing streak, monthly/exposure attribution, profit concentration, bootstrap CI for expectancy. Self-validated in `Tools/quantbeast_certification_selftest.py` against an existing, independently-computed 2026-07-16 reference (5 real trades) -- every formula matches exactly (profit factor 0.373869, max drawdown $308.29, 4-trade losing streak, etc.).
- **Critical infrastructure finding, Stage 3**: two real Strategy Tester probes (2019.06.03 and 2026.06.01) both returned `XAUUSD : real ticks begin from 2026.06.05 00:00:00, every tick generation used` -- this broker/demo feed's actual retained real-tick history is only ~7 weeks deep, not back to 2018 as bar-history metadata implied. Invalidated the entire original window selection; documented as a dated correction in `CERTIFICATION_WINDOW_SELECTION.md` rather than silently replaced. Cross-referenced against the ledger's disqualified list: only ~19 clean real-tick days remain available (2026.06.13-14, 2026.06.27-28, 2026.07.04-18).
- User's response: a hybrid Tier A/B/C/D architecture -- Tier A (Model=1 generated-tick structural screen across the original 2019-2024 windows, explicitly labeled `LOWER_FIDELITY_MODEL1_STRUCTURAL_SCREEN`, stronger provisional gates), Tier B (Model=1 vs Model=4 bridge validation on 2 real-tick-era windows, trade-level comparison, verdict `MODEL1_PROXY_ACCEPTABLE/DIRECTIONAL_ONLY/UNRELIABLE`), Tier C (real-tick test on the ~19 clean days, expect underpowered/INCONCLUSIVE), Tier D (demo-forward, kept separate from historical evidence, never pooled). Tasks #26-29 created; Tier B (bridge validation) started.
- **Tier B's first two probe runs surfaced a real, live-blocking bug, not a data problem**: both showed 100% signal rejection with "Max pending orders: 0". Traced to `CRiskEngine::ValidateTrade()` (`RiskEngine.mqh:308`) applying its pending-orders cap unconditionally; `InpMaxPendingOrders=0` (the canonical roster's own value) made `0>=0` permanently true. **This meant the live canonical-roster deployments earlier today (`qb-live-20260724-02`, `qb-live-20260724-05-longrun`) were very likely structurally unable to place a single trade for their entire runtime** -- a defect entirely unrelated to strategy edge, found only as a side effect of the certification sprint's own paranoia about trusting results before verifying the pipeline. See `DECISION_LOG.md` D018 for the full record.
- **Fixed and redeployed live, per explicit user authorization**: added a `marketOrdersOnly` parameter to `ValidateTrade()` (skips the pending-cap check when the account can only ever route market orders); new TEST 108 proves both the fix and that the cap still protects when pending routing is genuinely possible. Compiled 0/0. Diagnostic self-test-only run: 111/0 (up from 110). Re-ran the exact probe that surfaced the bug on the post-fix binary: a real FBO signal was accepted and executed (`SHADOW: FBO ORDER_TYPE_BUY lots=0.01 entry=4519.16 sl=4511.26 tp=4533.38`), hit its stop loss, and `InpMaxConsecLosses=1` correctly blocked further entries afterward -- proving the fix and that the other risk controls behave exactly as designed once this defect is out of the way. Redeployed as `qb-live-20260724-06-pendingcapfix` (14-day lease, terminal restart + fresh manual attach per D017's procedure, `preflight`/`deploy`/`verify` all clean). Confirmed directly on the live attach's own Experts log: `TEST 108 PASS`, `Self-tests complete: 111 passed, 0 failed`.
- Files changed: `Include/QuantBeast/Risk/RiskEngine.mqh`, `Experts/QuantBeast/QuantBeastEA.mq5`, `Include/QuantBeast/Testing/SafetyTests.mqh`, new `Tools/quantbeast_certification.py` / `Tools/quantbeast_certification_selftest.py`, new `TestEvidence/level1_edge_certification_20260724/` evidence pack, plus this entry and `DECISION_LOG.md` D018.
- Next task: resume Tier B (Model=1 vs Model=4 bridge validation) now on the fixed binary -- `qb-live-20260724-06-pendingcapfix` is the standing live deployment going forward, superseding `-05-longrun`. Tasks #26 (in progress) through #29 (final verdict) remain open; see the task tracker.

### 2026-07-25 -- Tier B bridge result (MODEL1_PROXY_UNRELIABLE) + journal-collision root-cause fix (D019)

- **Tier B verdict**: `MODEL1_PROXY_UNRELIABLE`. Two independent real-tick-era windows (2026.07.06-08, 2026.07.13-15), identical frozen canonical roster, Model=4 vs Model=1. Model=4 produced genuine trades (1 and 2 respectively, normal stop/target exits). Model=1 produced **zero trades in both** -- its ~59-72x sparser generated ticks (11k vs 648k-795k for the same window) create unrealistic price jumps that trip `InpMaxPriceJumpPoints`/data-quality preflight gates 974 and 978 times against only 552 bars each, dominating the run before strategy logic is ever reached. Per the user's explicit instruction: Tier A's original broad Model=1 screen does not proceed; the original Model=1 plan is marked `INVALIDATED_MODEL1_PROXY_FAILURE`. See `TestEvidence/level1_edge_certification_20260724/TIER_B_BRIDGE_VALIDATION.md`.
- **Journal-collision root cause found and fixed (D019)**: traced Tier B's disclosed journal-write-loss limitation to `OpenJournalFile()` (`Diagnostics.mqh`) sharing one filename across all runs with write-exclusive-only access (`FILE_SHARE_READ`, no `FILE_SHARE_WRITE`) -- rapid successive Tester runs collided (`error=5004`, 332 occurrences that day, 308 of them `TPOutcomeJournal.csv` alone). Fixed with per-run auto-generated unique journal filenames (new `InpJournalRunTag` input, honored when set; auto-fallback tag using real wall-clock time + tick count whenever `MQLInfoInteger(MQL_TESTER)` is true and no explicit tag arrived). Discovered along the way: string-type inputs do not reliably apply via the Tester's `--inputs_path` .set mechanism (confirmed empirically, both quoted and unquoted) -- worked around by gating the auto-tag on the runtime `MQL_TESTER` fact rather than any input. `WriteCSVLine()` now verifies the file actually grew and logs loudly if not (previously `LogSignal()` hardcoded `return true` regardless). Certification runs (explicit tag) fail closed if the journal can't open; live/production is unaffected (empty tag, unchanged filenames/behavior). New TEST 109 proves isolation; a real back-to-back integration test confirmed the collision and `TPOutcomeJournal.csv`'s near-constant failures are gone, and that the resulting tagged `TradeJournal_auto....csv` genuinely contains the real trade data.
- Compile 0/0 (twice -- first pass had two `ulong`/`long` conversion warnings, fixed). Self-tests: 112 passed, 0 failed (up from 111).
- Files changed: `Include/QuantBeast/Core/Diagnostics.mqh`, `Include/QuantBeast/Core/Configuration.mqh`, `Include/QuantBeast/Analytics/TradeJournal.mqh`, `Include/QuantBeast/Analytics/TPOutcomeTracker.mqh`, `Include/QuantBeast/Analytics/CounterfactualTracker.mqh`, `Experts/QuantBeast/QuantBeastEA.mq5`, `Include/QuantBeast/Testing/SafetyTests.mqh`, plus this entry, `DECISION_LOG.md` D019, and `TestEvidence/level1_edge_certification_20260724/TIER_B_BRIDGE_VALIDATION.md`.
- Next task: proceed to Tier C -- real-tick-only test across the ~10 genuinely clean trading weekdays in 2026.07.06-07.17 (combined roster + BO/FBO/MR/TPV2 individually), using the fixed journal mechanism for real CSV evidence this time. Expected verdict absent a clear negative result: `REAL_TICK_EDGE_CERTIFICATION_INCONCLUSIVE` (sample size). No advanced statistics (DSR/PBO/walk-forward/Monte Carlo) until a trustworthy larger sample exists -- explicitly deferred, not built this pass. Note this live-attached `qb-live-20260724-06-pendingcapfix` was NOT recompiled/redeployed with the D019 journal fix (it doesn't use `InpJournalRunTag`, and journal-write-loss was never a live-path risk -- only rapid Tester automation triggers the collision), so no redeploy action is needed for this fix specifically.

### 2026-07-25 (later) -- Tier C real-tick test + Level-1 Edge Certification Sprint final verdict

- **Tier C run**: all 5 configurations (combined, BO-only, FBO-only, MR-only, TPV2-only) run once each across the full 2026.07.06-2026.07.18 real-tick window (~3.56M ticks/run, 33-48 min each). Results: **5 distinct real trades total** -- BO 3 (SHORT, +0.34/+1.30/-7.14), FBO 1 (LONG, -6.92), MR 1 (LONG, -5.44), TPV2 0 (consistent with its entire prior history of zero organic signals). Combined's 1 trade duplicates the FBO-only trade (FBO fires first in this window; its loss triggers `InpMaxConsecLosses=1` and locks the combined run for the rest of the 12 days) -- not a 6th independent data point. Full trade-level detail, rejection attribution, and descriptive-only analysis (execution-pipeline correctness, strategy reachability, rejection distribution, per-strategy contribution, long/short behavior, catastrophic-behavior check, consistency with prior evidence -- explicitly no edge claim) in `TestEvidence/level1_edge_certification_20260724/TIER_C_REAL_TICK_TEST.md`.
- **Journal auto-tag found only partially reliable in practice**: 5 of 7 real Tester invocations against the fixed build resolved an empty tag at runtime despite `MQLInfoInteger(MQL_TESTER)` being unconditionally true and TEST 109 proving the logic correct in-process -- not a new collision in this specific run sequence (each affected run was a different single-strategy config, separable by the `Strategy` CSV column), but not yet trustworthy for same-config back-to-back runs. Text-log parsing (proven reliable throughout Tier B and C) was used as the primary source for every Tier C number instead. Disclosed as an open item for a future investigation pass, not silently claimed fixed.
- **Evidence classification recorded**: original Tier A (Model=1) plan `INVALIDATED_MODEL1_PROXY_FAILURE`; `qb-live-20260724-02`/`qb-live-20260724-05-longrun` `INVALID_FOR_EDGE_PENDING_CAP_DEFECT` (ran with the D018 bug -- their "0 trades" reflects the defect, not market absence); `qb-live-20260724-06-pendingcapfix` is the first valid forward-evidence deployment.
- **Advanced statistics (DSR/PBO/walk-forward/Monte Carlo) explicitly deferred, not built** -- a 5-trade sample is two orders of magnitude below the sprint's own >=100-trade gate; running them anyway would manufacture false statistical precision.
- **Final ticket verdict: `LEVEL1_EDGE_CERTIFICATION_INCONCLUSIVE_DATA_CONSTRAINT`** -- not failed (nothing catastrophic or clearly negative in the sample), not certified (nowhere near enough data in any dimension the sprint's own gates require). Separately: `EXECUTION_PIPELINE_STATUS=VERIFIED_CORRECT`, `MODEL1_PROXY_STATUS=UNRELIABLE`, `REAL_TICK_SAMPLE_STATUS=SEVERELY_UNDERPOWERED`, `DEMO_FORWARD_STATUS=IN_PROGRESS_NOT_YET_SUFFICIENT`.
- **Separate follow-up ticket created** (not started, not authorized to proceed): `TestEvidence/level1_edge_certification_20260724/FOLLOWUP_HISTORICAL_DATA_ACQUISITION.md` -- evaluating an independent XAUUSD tick dataset as an MT5 custom symbol, explicitly kept out of this sprint's evidence/verdict pending provenance/symbol-economics/timezone/spread/Coinexx-mapping validation.
- Trading-behavior impact: none -- Tier C was Shadow-mode only, zero broker orders transmitted; `qb-live-20260724-06-pendingcapfix` untouched, continues accumulating independent Level-2 demo-forward evidence.
- Files affected: `TestEvidence/level1_edge_certification_20260724/TIER_C_REAL_TICK_TEST.md` (new), `TestEvidence/level1_edge_certification_20260724/FOLLOWUP_HISTORICAL_DATA_ACQUISITION.md` (new), plus this entry and `DECISION_LOG.md` D020.
- Next task: this closes the Level-1 Edge Certification Sprint as scoped. Ongoing: (1) let `qb-live-20260724-06-pendingcapfix` keep accumulating Level-2 demo-forward evidence toward the sprint's own required sample (>=50 overall, >=10 per active strategy); (2) investigate the journal auto-tag intermittency if it matters for a future certification pass; (3) the historical-data-acquisition ticket remains proposed-only.
