...
- Pending-order lifecycle simulation (place, fill/cancel, stop) is now implemented in the Shadow layer with deterministic test coverage. Live/broker pending-order recovery, activation, expiry modeling, and organic market-fill behavior remain unproven.
...
- As of 2026-07-20: `OnTradeTransaction()`'s live entry-handling comment parsing (`QuantBeastEA.mq5:1741-1742`) does not match `PositionManager.mqh`'s `StrategyFromComment()` (used by restart's `ReconstructFromBroker()`) -- the live path does not truncate at a second `_` in the comment, so a comment like `QB_FBO_fixture` resolves to an unrecognized strategy id via a live fill but correctly to `FBO` via restart. Affects strategy-attribution/journaling only, not order safety. Medium severity; not yet fixed.
