...
- Startup fixtures have captured runtime output: 45 passed, 0 failed, including deterministic regime and arbitration ranking, duplicate, opposing, exposure, lower-ranked rejection coverage, final-decision signal writing, performance updates with file trade journaling disabled, live-mode strategy/execution gates, state symbol scoping, live recovery no-passive-flatten gating, unknown-position no-adoption behavior, alert-routing behavior, entry preflight controls, and session/rollover exit policy.
