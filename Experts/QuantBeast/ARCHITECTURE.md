...
| Strategies | BO, FBO, TP, MR | Substantive | Real long/short methods plus additive, default-preserving trigger-mode (break-retest / probe-confirm / displacement / rejection), level-source, stop-mode, and target-mode variations selectable per strategy; emitted signals now carry family/template/tag metadata for later batch validation and ML use; deterministic reachability + mode tests pass. |
