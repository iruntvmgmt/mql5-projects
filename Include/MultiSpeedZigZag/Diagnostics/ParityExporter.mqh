...
public:
   ...
   {
      int h=OpenCsv("manifest","key;value");
      if(h==INVALID_HANDLE) return false;
      FileWrite(h,"engine_version",engine_version);
      FileWrite(h,"commit_sha",commit_sha);
      FileWrite(h,"symbol",symbol);
      FileWrite(h,"timeframe",EnumToString(timeframe));
      ...
      FileWrite(h,"fast_atr_len",fast_len);
      FileWrite(h,"fast_atr_mult",DoubleToString(fast_mult,8));
      FileWrite(h,"medium_atr_len",med_len);
      FileWrite(h,"medium_atr_mult",DoubleToString(med_mult,8));
      FileWrite(h,"slow_atr_len",slow_len);
      FileWrite(h,"slow_atr_mult",DoubleToString(slow_mult,8));
      FileWrite(h,"min_pivot_spacing",min_spacing);
      FileWrite(h,"reversal_source",reversal_source);
      FileWrite(h,"breakout_source",breakout_source);
      FileWrite(h,"geometry_mode",geometry_mode);
      FileFlush(h);
      FileClose(h);
      return true;
   }
