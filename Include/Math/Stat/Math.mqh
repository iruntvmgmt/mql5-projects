...
//+------------------------------------------------------------------+
//| Checks probability input and converts tail/log_mode to lower CDF |
//+------------------------------------------------------------------+
bool MathCheckProbabilityInput(const double probability,const bool tail,const bool log_mode,double &prob,int &error_code)
  {
//--- in log mode -inf is a valid probability and means p=0
   if(!MathIsValidNumber(probability))
     {
      if(!(log_mode && probability==QNEGINF))
        {
   ...
   prob=TailLogProbability(probability,tail,log_mode);
...
//+------------------------------------------------------------------+
//| Returns true when bisection interval is small enough              |
//+------------------------------------------------------------------+
bool MathBisectionConverged(const double lo,const double hi,const double abs_tol,const double rel_tol)
  {
   double mid=0.5*(lo+hi);
   double width=hi-lo;
   double scale=MathMax(1.0,MathAbs(mid));
   return(width<=abs_tol || width<=rel_tol*scale);
  }
