...
with open(f"{OUT}/corrected_partial_fraction_summary_rerun.csv", "w", newline='') as f:
    w = csv.DictWriter(f, fieldnames=fieldnames)
    w.writeheader()
    for r in rows_out:
        w.writerow(r)

print(f"\nWrote {OUT}/corrected_partial_fraction_summary_rerun.csv ({len(rows_out)} variants)")
sys.exit(0)
