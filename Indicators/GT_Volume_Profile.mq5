...
#property description ":: God Tier Volume Profile — v7.41 (Stable) ::"
...
#property description "v7.41: Fixed enum ordering, DBL_MAX, duplicate g_priorPOC"
...
// ── Prior session VA ────────────────────────────────────────────────────────
double g_priorVAH = 0, g_priorVAL = 0, g_priorPOC = 0;
...
void CalcRotationFactor()
{
   ...
   if(g_priorPOC > 0 && g_POC > 0)
   {
      if(g_POC > g_priorPOC) g_rotationUp++;
      else if(g_POC < g_priorPOC) g_rotationDown++;
   }
   g_priorPOC = g_POC;
...
void UpdateZZSwingExtremes(const double &h[], const double &l[], int idx, double atr)
{
   ...
   if(g_zzHighExtreme > 0 && l[idx] <= g_zzHighExtreme - reversalThreshold
   ...
   // Confirm swing low: price rallies ATR*mult from extreme
                    ...
                    const double &o[], int idx, double atr)
{
   ...
   // BOS Bull: in uptrend (bias >= 0), price breaks above last swing high
   if(g_zzStructBias >= 0 && breakPrice > refHigh)
   {
      g_zzBOSConfirmCount++;
      if(g_zzBOSConfirmCount >= InpBOSConfirmCandles)
         { g_zzBullBOS = true; g_zzBOSConfirmCount = 0; }
   }
   ...
   else if(g_zzStructBias >= 0 && breakPriceBear < refLow)
   {
      g_zzBOSConfirmCount++;
      if(g_zzBOSConfirmCount >= InpBOSConfirmCandles)
         { g_zzBearCHoCH = true; g_zzBOSConfirmCount = 0; }
   }
   // BOS Bear: in downtrend (bias <= 0), price breaks below last swing low
   else if(g_zzStructBias <= 0 && breakPriceBear < refLow)
   {
      g_zzBOSConfirmCount++;
      if(g_zzBOSConfirmCount >= InpBOSConfirmCandles)
         { g_zzBearBOS = true; g_zzBOSConfirmCount = 0; }
   }
   ...
   else if(g_zzStructBias <= 0 && breakPrice > refHigh)
   {
      g_zzBOSConfirmCount++;
      if(g_zzBOSConfirmCount >= InpBOSConfirmCandles)
         { g_zzBullCHoCH = true; g_zzBOSConfirmCount = 0; }
   }
   else
...
int OnInit()
{
   ...
   IndicatorSetString(INDICATOR_SHORTNAME, "GT VP v7.41");
   g_fvgCount = 0;
   g_cloudTileCount = 0;
   ...
   g_rotationUp = 0; g_rotationDown = 0; g_priorPOC = 0;
   ...
   g_zzStructBias = 0; g_zzBOSConfirmCount = 0;
   g_zzScoreLong = 50.0; g_zzScoreShort = 50.0;
...
{
   if(rates_total < InpVPLookback + 10) return 0;
   ...
   g_isNewSession = forceFull || IsNewSession(t[rates_total-1], prevBarTime);
   ...
   if(g_isNewSession)
   {
      ...
      {
         g_priorVAH = g_VAH; g_priorVAL = g_VAL; g_priorPOC = g_POC;
         g_hasPriorVA = true;
      }
      // Snapshot session bar count for cloud spawn calibration
      if(g_sessionStartBar > 0)
         g_prevSessionBars = MathMax(1, rates_total - 1 - g_sessionStartBar);
      // Reset session state
   ...
   // ══════════════════════════════════════════════════════════════════════
   // HEAVY WORK — gated to new-bar-close only
   // ══════════════════════════════════════════════════════════════════════
   if(isNewBar || forceFull)
   {
      datetime now = t[rates_total-1];
...
void DrawPriorVA(const datetime &t[], int leftBar, int rightBar)
{
   ...
   if(g_priorPOC > 0)
   {
      ...
      if(ObjectFind(0, pocName) < 0)
         ObjectCreate(0, pocName, OBJ_TREND, 0, tLeft, g_priorPOC, tRight, g_priorPOC);
      else
      {
         ObjectSetInteger(0, pocName, OBJPROP_TIME, 0, tLeft);
         ObjectSetDouble(0,  pocName, OBJPROP_PRICE, 0, g_priorPOC);
         ObjectSetInteger(0, pocName, OBJPROP_TIME, 1, tRight);
         ObjectSetDouble(0,  pocName, OBJPROP_PRICE, 1, g_priorPOC);
      }
