...
- **Correctness details caught during design review**: R-multiple computed from `intent.average_fill_price` (the true cost basis), not `intent.requested_entry` (the pre-submission candidate price) — using the wrong one would have silently produced systematically-wrong R values on every trade. Realized-profit convention matches D013's `AccountSafeguard.mqh` (`DEAL_PROFIT+DEAL_SWAP+DEAL_COMMISSION`). MFE/MAE window uses the position's actual fill time, not `intent.signal_time` (one bar earlier).
