...
**Track 2 — Edge Discovery Sprint** (D016–D017, Stage A infrastructure): both halves of Stage A's stated output now exist — per-trade CSV (D016) and a master run-level summary CSV (D017). Next step toward Stage A itself: actually running the 8-strategy × 4-exit-multiple matrix on XAUUSD M5 at canonical ATR settings and reviewing the results before touching parameters or adding strategies, per the user's own instruction.

## Validated 2026-07-26 (twelfth pass) — Edge Discovery Sprint, Stage A infrastructure: master run-level summary CSV (D017)

Second half of Stage A's stated output. D016 built per-trade export; this pass builds the run-level aggregate: one row per completed Tester run, with win rate, expectancy, profit factor, max drawdown, average MFE/MAE, average holding time, and a long/short split, plus run metadata.

- **Extended, not new**: `CMSZZTradeAnalyticsExporter` (D016) now accumulates every trade it exports into small internal arrays, fed by the same `ExportClosedTrade()` call D016 already makes — no second detection pass, no risk of two independently-computed R-values silently disagreeing. A new `CMSZZRunSummaryPolicy` (pure, static, array-based) computes `Average`, `WinRate` (a breakeven trade does not count as a win), `ProfitFactorR` (returns a documented `-1.0` sentinel for "wins but zero losses" rather than a meaningless infinity), and `MaxDrawdownR` (walks the cumulative-R equity curve for the largest peak-to-trough drop).
- **Wiring**: a new `WriteRunSummary()` method, called once from `OnDeinit()`, aggregates the accumulated arrays and writes one row to a continuously-appended `MSZZ_RunSummary.csv` — a no-op (writes nothing) when zero trades occurred, matching D016's own "nothing to report" behavior.
- **Compile**: EA 0 errors, 1 pre-existing reviewed warning. Changed file (`TradeAnalyticsExporter.mqh`) and new test file 0 errors/0 warnings. Live tree and isolated instance hash-verified identical.
- **Tests**: 10/10 deterministic assertions, `failures=0` — `Average` on a known set and the empty-set edge case; `WinRate` with a mix including an exact-zero breakeven (correctly not counted as a win) and an all-wins case; `ProfitFactorR` for a normal mix, the all-wins sentinel, and the all-losses case; `MaxDrawdownR` for a monotonically-improving curve (zero drawdown), a single large loss after wins, and a genuine peak-trough-recovery shape.
- **Shadow regression**: short and long windows both `successfully finished`, 0 orders/0 deals/0 trades, identical 431/178 counts on the long window, zero `MSZZ_RunSummary.csv` (or `MSZZ_TradeAnalytics.csv`) files created in either run — correct, since zero trades means `WriteRunSummary()` has nothing to report.
- **Full regression**: all twelve unit-test suites re-ran clean, `failures=0` each.
- **Correction made during this pass**: D016's own decision-log and handoff entries had described a raw-currency realized-profit calculation (`DEAL_PROFIT+DEAL_SWAP+DEAL_COMMISSION`) that was never actually implemented — caught while designing this pass's profit-factor logic and corrected in both documents (separate commit) rather than left standing. This entire series stays R-only; there is no dollar-denominated profit anywhere in the CSVs.
- **Not exercised, same caveat as D016**: this aggregation has never run against a real completed trade, on this branch or anywhere. That remains true until Stage A backtests are actually run.

See `DECISION_LOG.md` D017 for the full design rationale and rejected alternatives (a second independent history scan instead of reusing D016's already-computed values, incremental mid-run summary writes, a separate exporter file).