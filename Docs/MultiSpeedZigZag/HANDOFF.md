...
Awaiting an explicit go/no-go on Phase 13 (the first controlled demo execution test) per D014's conditional verdict. If go: follow D014's specific recommended conditions exactly (isolated instance, `InpMaxTradesPerDay=1` override, a real nonzero `InpMaxDailyLossAmount`, active supervision, immediate revert to shadow after the one trade, then a restart to verify reconciliation against the real history). If no-go, or if a stricter bar is wanted first: the next code increment would be closing the Phase 8 gap (wire `expiry_time` enforcement even though nothing currently reaches it, as defense-in-depth before any retry logic is ever added) or beginning Phase 5 (risk sizing) despite it not being assessed as a blocker.