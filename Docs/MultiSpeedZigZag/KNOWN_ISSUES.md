...
### FIXED 2026-07-25 (second pass)

- **Malformed cluster IDs**: fixed per DECISION_LOG.md D004. `OpportunityClusterEngine` now has dedicated `EncodeClusterId()`/`DecodeClusterId()` methods using a versioned, length-prefixed format (`MSZZC1|<len>:<symbol>|<len>:<timeframe>|<len>:<direction>|<len>:<origin_type>|<len>:<origin_id>`) that is parse-safe regardless of `|` or `:` characters inside `origin_id`. Verified with dedicated round-trip/malformed-input tests in `Test_MSZZ_Clusters.mq5` (origin IDs containing `|`, containing `:`, empty, 500 characters long, a real nested breakout-ID example, decode-rejects-legacy-format, decode-rejects-truncated/malformed input, determinism, and distinctness). **Cluster IDs produced before this fix (including the shadow-test evidence in `BACKTEST_LOG.md` from 2026-07-25's first pass) are legacy evidence only** — the format changed, the old IDs cannot be decoded by the new decoder (this is intentional, not a regression), and any consumed-event store containing old-format IDs must be cleared/migrated before the next shadow-testing session (no automatic migration is performed).
