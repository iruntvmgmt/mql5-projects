...
## Implemented and verified 2026-07-26 (twelfth entry) — Edge Discovery Sprint, Stage A infrastructure: master run-level summary CSV (D017)

Second half of Stage A's stated output, extending D016 rather than adding a new file: `CMSZZTradeAnalyticsExporter` now accumulates every trade it exports (fed by the same `ExportClosedTrade()` call D016 already makes), and a new `WriteRunSummary()`, called once from `OnDeinit()`, writes one aggregated row per run — win rate, expectancy, profit factor, max drawdown, average MFE/MAE, average holding time, long/short split, plus run metadata — to a continuously-appended `MSZZ_RunSummary.csv`. The new pure `CMSZZRunSummaryPolicy` class does the actual math (`Average`, `WinRate`, `ProfitFactorR`, `MaxDrawdownR`), unit-tested the same deterministic way as every other policy class in this series.

Verified: EA and changed/new files compile clean (0 errors; EA carries the one pre-existing unrelated warning). 10/10 deterministic assertions pass, `failures=0`, including the documented `-1.0` profit-factor sentinel for "wins but zero losses" and correct peak-to-trough drawdown measurement on a recovery-shaped equity curve. Shadow regression (short + long windows) shows identical 431/178 counts and zero orders/deals/trades, with zero `MSZZ_RunSummary.csv` files created in either run (`WriteRunSummary()` is a documented no-op when zero trades occurred). Full regression (all twelve unit-test suites) all re-ran clean.

**A real documentation correction happened during this pass, not a code bug**: D016's own decision-log and handoff entries described reusing D013's `DEAL_PROFIT+DEAL_SWAP+DEAL_COMMISSION` convention for a realized-profit column — that was never actually implemented in D016's code, only in its description. This was caught while designing this entry's profit-factor logic (which needed to know whether a dollar-profit field existed to reuse) and corrected in both documents in a separate commit rather than silently carried forward. The entire trade-analytics/run-summary pipeline stays R-only throughout, by design, not by omission.

**Still not done, same caveat category as D016**: this aggregation logic has never executed against a real completed trade — no trade has ever closed on this branch. That remains unverified until Stage A backtests are actually run. `commit_sha` and ATR/date-range metadata beyond what's trivially derivable remain deferred, same reasoning as D016.

## Implemented and verified 2026-07-26 (eleventh entry) — Edge Discovery Sprint, Stage A infrastructure: trade-outcome analytics (D016)

`TradeAnalyticsExporter.mqh` is the first piece of a new, separate track from the execution-safety phases (D005–D015): the **Edge Discovery Sprint**, aimed at determining whether the strategy engine has any real edge, which the execution-safety plan never tried to answer. This entry builds the data-capture foundation — nothing in this codebase previously computed or exported anything about a trade after it opened. A new `DetectClosedPositions()` runs every bar, checking every `MSZZ_INTENT_POSITION_ACTIVE` intent's ticket directly (not via the D009 reconciler, which solves a harder identity-ambiguity problem this case doesn't have); on detecting a closed position it transitions the intent via the D010 state machine and exports one row (R-multiple, MFE/MAE-in-R, exit reason, bars held, session) to a continuously-appended `MSZZ_TradeAnalytics.csv`.

This closes a real architectural gap, not just adds a feature: the existing D009 reconciler only runs once at `OnInit()`, so in a single continuous Strategy Tester run spanning months, a position that opened and closed mid-test would never have been detected as closed at all under the prior architecture.

Verified: EA and new files compile clean (0 errors; EA carries the one pre-existing unrelated warning). 15/15 deterministic assertions pass, `failures=0`. Shadow regression (short + long windows) shows identical 431/178 counts and zero orders/deals/trades, with zero `MSZZ_TradeAnalytics.csv` files created in either run (correct — shadow mode never creates intents). Full regression (all eleven unit-test suites) all re-ran clean.

**Scope explicitly not covered by this decision**: the master run-level summary CSV (trades/win-rate/profit-factor/expectancy/drawdown per run with metadata); `spread_at_entry` and ATR/fast-medium-slow-structure-snapshot columns from the user's full wishlist schema — these are only known at signal time, not close time, and would require new plumbing (a schema-versioned `ExecutionIntentStore` bump or a side-table) to carry that context forward through the intent's lifetime; `commit_sha` in the CSV, since MQL5 has no runtime git access (recorded externally per research batch instead).

**Still not done, and honestly flagged as different in kind from every prior "not exercised" caveat in this series**: this component's entire purpose is to observe a closed trade, and there has never been one on this branch or anywhere in this session — so unlike D009–D015 (whose guards at least ran correctly in shadow mode against real account state, just never against a real order), this export logic has literally never executed against a real closed position. That remains unverified until Stage A backtests are actually run.

## Implemented and verified 2026-07-26 (tenth entry) — Stale-signal expiry enforcement, Phase 8 (D015)

Closes the Phase 8 gap D014 named, per the user's explicit choice to close it before any Phase 13 decision. The fix required more than adding an enforcement check: `MSZZCandidate.expiry_time` had never been assigned a nonzero value anywhere in `StrategySuite.mqh`'s shared `AddCandidate()` helper, so `OpportunityClusterEngine.mqh`'s already-correct expiry-aggregation logic had nothing real to aggregate. `AddCandidate()` now computes `expiry_time` from a new configurable `InpSignalValidityBars` (default `3`), and `ExecuteCluster()` rejects (`REJECT_EXPIRED`) against the cluster-level aggregated expiry as its very first content check, via a new static `CMSZZExecutionGuard::IsExpired()`.

Verified: EA and changed/new files compile clean (0 errors; EA carries the one pre-existing unrelated warning). 6/6 deterministic boundary assertions pass, `failures=0`. Critically, `Test_MSZZ_Clusters.mq5` (22 assertions on candidate/cluster construction, unchanged since this session's early passes) re-ran clean after the `AddCandidate()` change, confirming no existing candidate/cluster behavior regressed. Shadow regression (short + long windows) shows identical 431/178 counts and zero orders/deals/trades, with zero `REJECT_EXPIRED` lines in either run. Full regression (all ten unit-test suites) all re-ran clean.

**Known, accepted characteristic of this fix, not a limitation**: the `REJECT_EXPIRED` path is not expected to ever actually fire under the EA's current architecture. Candidates are generated and consumed synchronously within the same `OnTick()` call, with no queue or retry mechanism anywhere in the codebase, so `TimeCurrent()` at the check point is effectively identical to the candidate's own `signal_time` in every currently-possible code path. This was built and accepted as defense-in-depth for the day retry/backoff logic is added, per the user's explicit direction, not as a currently-active safety net.

**Still not done**: Phase 5 (risk sizing beyond fixed lots) remains unstarted, still assessed as a non-blocker per D014. D014's core finding — no live-execution code path has ever run against real broker state — is unchanged by this entry.

## Evaluated 2026-07-26 (ninth entry) — Phase 12 demo-readiness gate evaluation (D014)

No code changed. This is a cross-cutting synthesis of every "not exercised" caveat already stated individually in D009–D013, named explicitly because it is the single fact that determines what "the gates pass" can honestly mean: **no live-execution code path built since D009 has ever run against real broker state.** The reconciler's broker-record collection, the protection guard's repair, the margin guard's `OrderCalcMargin` call, and the account safeguard's deal-history query have each only ever executed in two contexts — a deterministic unit test against hand-constructed mock data, and a shadow regression run where `ExecuteCluster()` returns before any of them are even called. Full verdict, evidence walkthrough, and recommended conditions for a first narrow test are in `DECISION_LOG.md` D014; this entry exists so a reader of this file alone (without cross-referencing the decision log) still sees the same headline caveat.

Also confirmed by direct source inspection during this evaluation: `intent.expiry_time` (Phase 8, stale-signal revalidation) is populated at intent-creation time but is never read anywhere else in the codebase — there is no code path today that could act on a stale signal, since candidates are generated and consumed synchronously in the same `OnTick()` call with no queue or retry mechanism. This makes the gap currently unreachable, not currently safe by design — it would become load-bearing the moment any retry/backoff logic is added, and should be closed before that happens, not after.
